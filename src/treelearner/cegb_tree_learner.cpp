...
// mostly taken from SerialTreeLearner; only changed to store SplitInfo for all features
void CEGBTreeLearner::FindBestThresholds() {
  ...
  for (int feature_index = 0; feature_index < num_features_; ++feature_index) {
    ...
    smaller_all[feature_index].feature = train_data_->RealFeatureIndex(feature_index);


    leaf = smaller_leaf_splits_->LeafIndex();
    leaf_feature_penalty[leaf][feature_index] = CalculateOndemandCosts(train_data_->RealFeatureIndex(feature_index), leaf);
    ...
    larger_all[feature_index].feature = train_data_->RealFeatureIndex(feature_index);
...
double CEGBTreeLearner::CalculateOndemandCosts(int feature_index, int leaf_index) {
  ...
  for (data_size_t i_input = 0; i_input < cnt_leaf_data; ++i_input) {
  	if (coupled_features_used[train_data_->num_data() * feature_index + tmp_idx[i_input]])
  	  continue;
    total += penalty;
  }
...
void CEGBTreeLearner::FindBestSplitForLeaf(int leaf) {
	...
	for (int i_feature = 0; i_feature < num_features_; i_feature++) {
		...
		// FIXME:
		// a "good" split can have positive i_gain but negative i_gain - tradeoff * i_penalty
...
void CEGBTreeLearner::FindBestSplitsForLeaves() {
	...
	// need to update all leaves if branches are coupled and a new feature was used
...
void CEGBTreeLearner::Split(Tree* tree, int best_leaf, int* left_leaf, int* right_leaf)
{
  ...
  for (data_size_t i_input = 0; i_input < cnt_leaf_data; ++i_input)
  	coupled_features_used[train_data_->num_data() * inner_feature_index + tmp_idx[i_input]] = true;
